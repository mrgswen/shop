				...
				<td style="padding: 30px; text-align: center"><table
						...
						<tr>
							...
							<td style="padding-top: 30px"><font
								style="font-weight: bold; color: #FF0000"> <c:if
										test="${noUser != null }">
			              激活失败,无此用户!
          </c:if> <c:if test="${activeSuccess != null }">
                                                激活成功，请登录
          </c:if> <c:if test="${status!=null }">
                                       亲！您还没登录请先登录。
          </c:if> <c:if test="${registerStatus!=null }">
                                      注册成功，请到邮箱激活。
          </c:if> <c:if test="${paySuccessfully!=null }">
             <table>
             <tr><td colspan="2">支付成功</td></tr>
             </table>
          </c:if>
								...
								href="${ pageContext.request.contextPath }/login">登录</a></td>
						</tr>
					</table>
					<h1>&nbsp;</h1></td>
			</tr>
